document.write(`
  <!-- Modern Footer with Integrated Sections -->
   <footer class="modern-footer">
       <div class="footer-content">
         <div class="footer-top">
           <!-- Contact Column -->
           <div class="footer-column">
             <h3>Contact Us</h3>
             <ul class="footer-contact-list">
               <li>
                 <i class="fa-solid fa-location-dot"></i>
                 <span>Port Harcourt, Rivers, Nigeria</span>
               </li>
               <li>
                 <i class="fa-solid fa-phone"></i>
                 <span>+234 801 234 5678</span>
               </li>
               <li>
                 <i class="fa-solid fa-envelope"></i>
                 <span>info@habitique.com</span>
               </li>
               <li>
                 <i class="fa-solid fa-clock"></i>
                 <span>Mon-Fri: 9am - 5pm<br>Sat: 10am - 4pm</span>
               </li>
             </ul>
           </div>
 
           <!-- Quick Links Column -->
           <div class="footer-column">
             <h3>Quick Links</h3>
             <ul class="footer-links-list">
               <li><a href="#">Shop Now</a></li>
               <li><a href="booking.html">Book a Consultation</a></li>
               <li><a href="#">Interior Design Services</a></li>
               <li><a href="blog-index.html">Blog & Inspiration</a></li>
               <li><a href="#">FAQ</a></li>
             </ul>
           </div>
 
           <!-- Services Column -->
           <div class="footer-column">
             <h3>Our Services</h3>
             <ul class="footer-links-list">
               <li><a href="booking.html">Interior Design</a></li>
               <li><a href="booking.html">Furniture Selection</a></li>
               <li><a href="booking.html">Space Planning</a></li>
               <li><a href="booking.html">Color Consultation</a></li>
               <li><a href="booking.html">Custom Furniture</a></li>
             </ul>
           </div>
 
           <!-- Newsletter Column -->
           <div class="footer-column">
             <h3>Stay Connected</h3>
             <p>Subscribe to get updates on our latest collections and exclusive offers.</p>
 
           <div class="newsletter-wrapper">
             <form class="newsletter-form">
               <div class="newsletter-input-container">
                 <input type="email" placeholder="Your email address" required>
                 <button type="submit" class="subscribe-btn">Subscribe</button>
               </div>
             </form>
             <p class="newsletter-note">By subscribing you agree to our <a href="">Privacy Policy.</a></p>
           </div>
 
 
             <div class="social-icons">
               <a href="#"><i class="fa-brands fa-facebook-f"></i></a>
               <a href="https://instagram.com/habitique"><i class="fa-brands fa-instagram"></i></a>
               <a href="https://pinterest.com/habitique"><i class="fa-brands fa-pinterest-p"></i></a>
               <a href="https://X.com/habitique"><i class="fa-brands fa-twitter"></i></a>
               <a href="https://youTube.com/habitique"><i class="fa-brands fa-youtube"></i></a>
             </div>
           </div>
         </div>
 
         <!-- Payment Methods -->
         <div class="payment-methods">
           <span class="payment-label">Payment Methods:</span>
           <div class="payment-icons">
             <i class="fa-brands fa-cc-mastercard"></i>
             <i class="fa-brands fa-cc-visa"></i>
             <i class="fa-brands fa-cc-paypal"></i>
             <i class="fa-brands fa-cc-amex"></i>
             <i class="fa-brands fa-apple-pay"></i>
             <i class="fa-brands fa-google-pay"></i>
           </div>
         </div>
 
         <div class="footer-divider"></div>
 
         <!-- Footer Bottom -->
         <div class="footer-bottom">
           <!-- Left: Logo (always visible) -->
           <div class="footer-logo">
             <a href="index.html" class="logo">
               <span class="logo-brown">HABITI</span><span class="logo-white">QUE</span>
             </a>
           </div>
 
           <div class="footer-links">
             <a href="#">Customer Service</a>
             <a href="#">Terms and Conditions</a>
             <a href="#">Privacy Policy</a>
             <a href="#">Vouchers and Discounts</a>
           </div>
 
           <div class="footer-legal">
             <span>&copy; 2025 by Theresa. All rights reserved.</span>
           </div>
         </div>
       </div>
     </div>
 
   </footer>
   `);